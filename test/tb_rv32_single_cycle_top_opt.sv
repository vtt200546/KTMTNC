`timescale 1ns/1ps
module tb_rv32_single_cycle_top_opt;
    `include "rv32_defs.vh"

    reg clk = 0, rst = 1;

    rv32_single_cycle_top_opt #(
        .IMEM_DEPTH(64),
        .DMEM_BYTES(256)
    ) uut (
        .clk(clk),
    ...
    initial begin
        uut.u_imem.mem[0]  = enc_i(5,   5'd0, 3'b000, 5'd1,  OPC_ITYPE);
        uut.u_imem.mem[1]  = enc_i(7,   5'd0, 3'b000, 5'd2,  OPC_ITYPE);
        uut.u_imem.mem[2]  = enc_r(7'b0000000, 5'd2, 5'd1, 3'b000, 5'd3, OPC_RTYPE);
        uut.u_imem.mem[3]  = enc_s(0,   5'd3, 5'd0, 3'b010, OPC_STORE);
        uut.u_imem.mem[4]  = enc_i(0,   5'd0, 3'b010, 5'd4,  OPC_LOAD);
        uut.u_imem.mem[5]  = enc_b(8,   5'd3, 5'd4, 3'b000, OPC_BRANCH);
        uut.u_imem.mem[6]  = enc_i(1,   5'd0, 3'b000, 5'd5,  OPC_ITYPE);
        uut.u_imem.mem[7]  = enc_j(8,   5'd6, OPC_JAL);
        uut.u_imem.mem[8]  = enc_i(99,  5'd0, 3'b000, 5'd7,  OPC_ITYPE);
        uut.u_imem.mem[9]  = enc_u(20'h12345,            5'd10, OPC_LUI);
        uut.u_imem.mem[10] = enc_u(20'h00000,            5'd8,  OPC_AUIPC);
        uut.u_imem.mem[11] = NOP;
        ...
        if ({uut.u_dmem.mem[3], uut.u_dmem.mem[2], uut.u_dmem.mem[1], uut.u_dmem.mem[0]} !== 32'd12) begin
        ...
        $display("tb_rv32_single_cycle_top_opt PASSED");
        $finish;
    end
endmodule
